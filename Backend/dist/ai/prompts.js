...
exports.getFactCheckPrompt = getFactCheckPrompt;
exports.getClaimExtractionPrompt = getClaimExtractionPrompt;
exports.getSourceCredibilityPrompt = getSourceCredibilityPrompt;
exports.getPatternDetectionPrompt = getPatternDetectionPrompt;
exports.getQuickSummaryPrompt = getQuickSummaryPrompt;
/**
 * Generate comprehensive fact-checking prompt
 */
function getFactCheckPrompt(content, sourceUrl, title, relatedSources) {
    const sourcesContext = relatedSources && relatedSources.length > 0
        ? `\n\nRelated news from trusted sources:\n${relatedSources.map((s, i) => `${i + 1}. ${s.name}: ${s.snippet || 'No snippet available'} (${s.url})`).join('\n')}`
        : '';
    return `
 ...
 */
function getClaimExtractionPrompt(content) {
    return `
 ...
 */
function getSourceCredibilityPrompt(sourceUrl, sourceName) {
    return `
 ...
 */
function getPatternDetectionPrompt(content) {
    return `
 ...
 */
function getQuickSummaryPrompt(content) {
    return `
