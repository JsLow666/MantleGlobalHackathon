// src/ai/scorer.ts
import { NewsSource, isTrustedSource, getDomainReputation } from './sources';

interface AIResponse {
  confidence?: number;
  red_flags?: string[];
  supporting_factors?: string[];
  concerning_factors?: string[];
...
export function calculateCredibilityScore(
  ...
  const domainRep = getDomainReputation(sourceUrl);
 ...
 */
function analyzeContentQuality(content: string, aiResponse: AIResponse): number {
 ...
 * Returns score from 0-1
 */
function analyzeCorroboration(relatedSources: NewsSource[]): number {
  ...
  const trustedCount = relatedSources.filter(s => isTrustedSource(s.url)).length;
 ...
 */
function calculateRedFlagPenalty(redFlags: string[]): number {
 ...
 */
function calculateSupportingBonus(supportingFactors: string[]): number {
  if (supportingFactors.length === 0) return 0;
...
export function calculateConfidenceLevel(
  ...
  if (sourceCount >= 5) confidence += 15;
  else if (sourceCount >= 3) confidence += 10;
  else if (sourceCount >= 1) confidence += 5;

  // Having a verifiable source URL increases confidence
  if (hasSourceUrl) confidence += 10;
  else confidence -= 15;
